...
namespace tritech {
    ...
    struct mtAliveMsg {
        ...
        void print() {
            printf("mtAliveMsg: node = %#x headTime_msec = %d motorPos = %d inCentre = %d centered = %d motoring = %d motorOn"
                   "= %d dir = %d inScan = %d noParams = %d sentCfg = %d\n", txNode, headTime_msec, motorPos, inCentre,
                   centered,
    ...
    struct mtHeadCommandMsg {
        mtHeadCommandMsg(uint16_t _nBins = 200, float _range = 10, float _VOS = 1500, uint8_t _angleStepSize = 32,
                         int _leftLimit = 1, int _rightLimit = 6399, uint8_t _contFlag = 0x83) :
                nBins(_nBins), range(_range), VOS(_VOS), angleStepSize(_angleStepSize), leftLimit(_leftLimit),
                rightLimit(_rightLimit), contFlag(_contFlag) {}
        ...
        std::vector<uint8_t> construct() {
            ...
            std::vector<uint8_t> msg = {
                    0x00, 0x40, 0x30, 0x30, 0x34, 0x43, 0x4C, 0x00, 0xFF, 0x02, 0x47, 0x13, 0x80, 0x02, 0x1D, contFlag,
                    0x23, 0x02,
                    0x99, 0x99, 0x99, 0x02, 0x66, 0x66, 0x66, 0x05, 0xA3, 0x70, 0x3D, 0x06, 0x70, 0x3D, 0x0A, 0x09,
                    0x28,
                    0x00, 0x3C, 0x00, 0x01, 0x00, 0xFF, 0x18, 0x51, 0x08, 0x54, 0x54, 0x5A, 0x00, 0x7D, 0x00, 0x19,
                    0x10,
                    0x8D, 0x00, 0x5A, 0x00, 0xE8, 0x03, 0x97, 0x03, 0x40, 0x06, 0x01, 0x00, 0x00, 0x00, 0x50, 0x51,
                    0x09,
                    0x08, 0x54, 0x54, 0x00, 0x00, 0x5A, 0x00, 0x7D, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0A};

            uint16_t rangeScale = floor(range * 10 + 0.5);
//            msg[35] = (rangeScale >> 8)<<1;
            msg[36] = rangeScale & 0x00FF;
            msg[37] = rangeScale >> 8;
//            msg[36] = rangeScale & 0x00FF;
//            msg[35] = rangeScale >> 8;
//            std::cout << rangeScale << std::endl;
//            std::cout << std::bitset<8>(msg[34]) << std::endl;
//            std::cout << std::bitset<8>(msg[35]) << std::endl;
//            std::cout << std::bitset<8>(msg[36]) << std::endl;
//            std::cout << std::bitset<8>(msg[37]) << std::endl;
//            std::cout << std::bitset<8>(msg[38]) << std::endl;
//            std::cout << std::bitset<8>(msg[39]) << std::endl;
            //4+16+32+64+128+256
//            500
//            00101000
//            00000000
...
//      RCLCPP_INFO("leftLimit"<<leftLimit);
            ...
            uint16_t rightLimGrads = rightLimit;//std::min(6399, std::max(1, int(rightLimit /*/ 360.0 * 400 * 16*/)));
    ...
    struct mtHeadDataMsg {
        ...
        enum sweepCode_t {
            Scanning_Normal, Scan_AtLeftLimit, Scan_AtRightLimit, Scan_AtCentre
        };
        ...
        float rangeScale;
        enum rangeUnits_t {
            meters = 0, feet = 1, fathoms = 2, yards = 3
        };
        ...
        void print() {
            ...
            std::cout << "   Continuos Scan?: " << headControl.cont << std::endl;
            std::cout << "   scanright?: " << headControl.scanright << std::endl;
            std::cout << "   invert?: " << headControl.invert << std::endl;
            std::cout << "   motoff?: " << headControl.motoroff << std::endl;
            std::cout << "   txoff?: " << headControl.txoff << std::endl;
            std::cout << "   spare?: " << headControl.spare << std::endl;
...
//			}
            ...
            if (headControl.adc8on) {
                ...
                std::cerr << __FUNCTION__ << ":" << __LINE__ <<
                          " - Your device is transmitting in packed 4-bit mode. This is not yet supported."
                          << std::endl;
                return;
            }
        }
